...
Roc_analysis_boot <- function(model, test_data,predicto ,resp, model_name = "Model") {
  ...
  # ROC curve with bootstrapping for confidence intervals. stratified = TRUE maintains the class proportions 
  
  roc_obj <- roc(test_data[[resp]], probabilities, levels = c("No", "Yes"), direction = "<", boot.n = 10000, stratified = TRUE)
  
  # Plotting ROC curve.
  plot(roc_obj, col = "red", lwd = 2, main = paste("ROC curve -", model_name), print.auc = TRUE)
  ...
  # Confusion matrix to evaluate classification performance, the "positive" argument specifies which class is considered positive
